...
def test_normalize_table_name_accepts_short_name() -> None:
    assert (
        bronze_io.normalize_table_name("user_wallets_raw")
...
def test_load_bronze_records_reads_jsonl(tmp_path: Path) -> None:
    ...
    table_dir.mkdir()
    rows = [
        {"user_id": "user_1", "balance": "10.00", "frozen_amount": "0.00"}
    ]
    _write_jsonl(table_dir / "data.jsonl", rows)

    loaded = bronze_io.load_bronze_records(
        "user_wallets_raw", base_dir=tmp_path
    )
