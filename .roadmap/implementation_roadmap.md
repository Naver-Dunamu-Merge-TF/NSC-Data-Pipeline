...
| SEC-005 | Blocked | P0 | SEC-003, SEC-004 | `.specs/decision_open_items.md` (D-018, D-044), `.specs/cloud/cloud_migration_rebuild_plan.md` | 서비스 프린시플 `run_as` 전환 + UC ACL 최소권한 적용 + 실행 성공 | L3 | `.agents/logs/verification/20260223_sec005_run_as_acl.md` | Infra + DataEng | G1 | 1) `run_as`/ACL 적용 스크립트 및 smoke poll 스크립트 구현 완료<br>2) apply 시도에서 대상 principal 미존재로 실패(`Invalid user`), D-044로 에스컬레이션<br>3) A/Silver/B/C smoke는 20초 polling/600초 timeout에서 미수렴(SEC-005 유지 블로커) |
