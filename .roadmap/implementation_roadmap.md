...
| SEC-004 | Blocked | P0 | SEC-001 | `.specs/cloud/cloud_migration_rebuild_plan.md`, `.specs/decision_open_items.md`, `scripts/phase7/sec003_sec004_binding_window.sh` | Key Vault-backed Secret Scope 접근 검증 및 rotation 절차 확인 완료(서버리스 SEC window 기준) | L3 | `.agents/logs/verification/YYYYMMDD_sec004_kv_scope_rotation_serverless.md` | Infra | G1 | 1) 기존 `ledger-analytics-dev` backend 점검 및 KV-backed 재생성<br>2) 서버리스 secret read probe 실행<br>3) rotation pre/post hash diff 증적 확보 |
| SEC-005 | Blocked | P0 | SEC-003, SEC-004 | `.specs/decision_open_items.md` (D-018, D-044, D-045, D-046), `.specs/cloud/cloud_migration_rebuild_plan.md` | 서비스 프린시플 `run_as` 전환 + UC ACL 최소권한 적용 + staged smoke(L3) 수렴 성공 + 증적 고정 | L3 | `.agents/logs/verification/20260223_all_pipeline_serverless_signoff.md` | Infra + DataEng | G1 | 1) A/Silver/B/C 서버리스 wiring 배포 및 runtime assertion PASS<br>2) SP+verifier ACL assertions PASS (`20260223_sec005_acl_assertions.json`, `20260223_sec005_verifier_acl_assertions.json`)<br>3) D-046 staged smoke 표준(`sync_dim_rule_scd2 -> A/Silver -> B/C`) 적용 및 `run_sec005_smoke_recovery.sh` 증적 확보(`20260224_d046_smoke_recovery_verification.md`) |
...
| SEC-007 | Blocked | P1 | SEC-003, SEC-006 | `.specs/cloud/cloud_migration_rebuild_plan.md#12.1`, `docs/plans/2026-02-24-d045-smoke-convergence-recovery.md` | Pipeline A 최소 1회 스모크 성공 + `silver.dq_status` 또는 Gold 산출물 생성 확인 + staged smoke PASS 증적 확보 | L3 | `.agents/logs/verification/20260224_d046_smoke_recovery_verification.md` | DataEng | G1 | 1) D-046 전환으로 SEC-005 smoke 판정 기준을 staged smoke로 고정<br>2) verifier fail-fast deterministic(`--fail-fast-deterministic on`)으로 retry pending 고착 케이스 조기 종료<br>3) cadence gate는 `sample_count=0` 유지, 성공 샘플 6건 확보 전까지 Blocked |
